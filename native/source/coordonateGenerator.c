...
Shape Square(double size){
    ...
    if (size >= currentWindow.WindowXsize || size >= currentWindow.WindowYsize) {
        s.count = 0;
        return s;
    }


    int LeftUpCorner_x = rand() % (currentWindow.WindowXsize - ((int)size + 1)); 
    int LeftUpCorner_y = rand() % (currentWindow.WindowYsize - ((int)size + 1));
...
Shape RandomWaves(size_t samples) {
    ...
    double A = rand() % (currentWindow.WindowYsize - currentWindow.WindowYsize / 2);
    double w = 0.009;
    double e = 0;
    int rectification = currentWindow.WindowYsize / 2;
